namespace :mellow do
  ...
  task rewards_route_check: :environment do
    ...
    puts "submitted_wallet: #{position.mellow_metadata_hash['submitted_wallet'].presence || position.wallet.address}"
    ...
    puts "ui_parity_candidate_contract: #{rewards[:ui_parity_contract_address] || MellowUiParityRewards::CONTRACT_ADDRESS}"
    puts "ui_parity_candidate_contract_role: #{rewards[:ui_parity_contract_role] || MellowUiParityRewards::CONTRACT_ROLE}"
    puts "ui_parity_selector: #{rewards[:ui_parity_selector] || MellowUiParityRewards::SELECTOR}"
    puts "ui_parity_selector_name: #{rewards[:ui_parity_selector_name] || MellowUiParityRewards::SELECTOR_NAME}"
    puts "ui_parity_verified_selector: #{rewards[:ui_parity_verified_selector].nil? ? 'unavailable' : rewards[:ui_parity_verified_selector]}"
    puts "ui_parity_call_from: #{rewards[:ui_parity_call_from] || 'unavailable'}"
    puts "ui_parity_call_to: #{rewards[:ui_parity_call_to] || rewards[:ui_parity_contract_address] || MellowUiParityRewards::CONTRACT_ADDRESS}"
    puts "ui_parity_wallet_arg: #{rewards[:ui_parity_wallet_arg] || 'unavailable'}"
    puts "ui_parity_raw_result: #{rewards[:ui_parity_raw_result] || 'unavailable'}"
    puts "ui_parity_decoded_aero: #{rewards[:ui_parity_decoded_aero] || 'unavailable'}"
    puts "ui_parity_delta_vs_expected: #{rewards[:ui_parity_delta] || 'unavailable'}"
    puts "ui_parity_delta_percent_vs_expected: #{rewards[:ui_parity_delta_percent] || 'unavailable'}"
    ...
    puts "chosen_reward_source: #{rewards[:reward_source] || 'gauge_route'}"
    puts "rewards_included_in_pnl: #{%w[estimated verified_zero detected].include?(rewards[:value_state].to_s)}"
    puts "candidate_reward_sources: #{rewards[:candidate_reward_sources].presence || 'none'}"
