namespace :ethereal do
  desc "Read-only Ethereal unified active-venue auto readiness diagnostics"
  task auto_readiness: :environment do
    position = ethereal_auto_position
    report = HedgeVenueAutoReadiness.new(
      adapters: { "ethereal" => HedgeVenueAutoAdapters::Ethereal.new }
    ).report(position: position)

    puts JSON.pretty_generate(report.merge(action: "ethereal_auto_readiness", orders_submitted: 0, signatures_created: 0))
  end

  desc "Run Ethereal active-venue one-shot auto rebalance; dry-run by default"
  task auto_rebalance_once: :environment do
    position = ethereal_auto_position
    live = ActiveModel::Type::Boolean.new.cast(ENV["live"].presence || ENV["LIVE"]) == true
    dry_run = if ENV.key?("dry_run") || ENV.key?("DRY_RUN")
      ActiveModel::Type::Boolean.new.cast(ENV.fetch("dry_run", ENV.fetch("DRY_RUN", "true")))
    else
      !live
    end
    result = HedgeVenueAutoRebalanceOnce.new(
      adapters: { "ethereal" => HedgeVenueAutoRebalanceAdapters::Ethereal.new }
    ).run(
      position: position,
      dry_run: dry_run,
      live: live,
    ...
    puts JSON.pretty_generate(result.receipt.merge(action: "ethereal_auto_rebalance_once"))
    abort("Ethereal auto rebalance blocked: #{result.blockers.join('; ')}") if live && !result.status.in?(%w[success submitted_and_confirmed no_op])
  end
  ...
  def ethereal_auto_position
    ...
    abort("Position #{position_id || '(missing)'} not found.")
  end
