  ...
  def check_previous_run
    ...
    if final_event.fetch("final_position_confirmed", nil) == false
      if short_size(@eth_position).zero?
        add_check(:previous_run, "Previous live/canary final position confirmed", false, warning: true, value: "current ETH readback nil")
      else
        add_check(:previous_run, "Previous live/canary final position confirmed", false, blocker: true, value: final_event.fetch("final_position", nil).inspect)
      end
    elsif final_event.fetch("final_position", nil).present?
      if short_size(@eth_position).zero?
        add_check(:previous_run, "Previous live/canary final position nil", false, warning: true, value: "current ETH readback nil")
      else
