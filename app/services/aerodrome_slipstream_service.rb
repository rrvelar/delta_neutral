...
#
# This service only performs eth_call requests. It never signs transactions,
# never requires private keys, and does not place approvals, transfers, swaps,
# NFT transfers, hedges, or Hyperliquid orders.
class AerodromeSlipstreamService
  ...
  def initialize(rpc_url: nil, position_manager_address: nil, factory_address: nil)
    @rpc_uri = rpc_uri(presence_or_env(rpc_url, "BASE_RPC_URL"))
  ...
  def rpc_uri(value)
    uri = URI(value)
    raise ConfigError, "Malformed Aerodrome RPC URL: #{value.inspect}" unless uri.is_a?(URI::HTTP) && uri.host.present?

    uri
  rescue URI::InvalidURIError
    raise ConfigError, "Malformed Aerodrome RPC URL: #{value.inspect}"
  end
  ...
  def int24_word(value)
    integer = Integer(value)
    raise DecodeError, "Expected int24 value, got #{value}" unless integer.between?(-(2**23), (2**23) - 1)
