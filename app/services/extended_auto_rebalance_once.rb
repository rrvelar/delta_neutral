class ExtendedAutoRebalanceOnce
  ...
  def build_plan(position:, current_position:, max_slippage:, probe_mode:, max_size_eth:, one_shot:)
...

    {
      ...
      strong_drift_threshold: anti_churn[:strong_drift_threshold],
      drift_to_tolerance_ratio: anti_churn[:drift_to_tolerance_ratio],
  ...
  def result(status:, blockers:, position:, plan:, current_position:, account_state:, signer_health:, conflict_state:, dry_run:, execution: nil)
    receipt = {
      ...
      strong_drift_threshold: plan[:strong_drift_threshold],
      drift_to_tolerance_ratio: plan[:drift_to_tolerance_ratio],
      ...
      execution_timing: execution && execution[:execution_timing],
      submit_latency_seconds: execution && execution[:submit_latency_seconds],
      readback_latency_seconds: execution && execution[:readback_latency_seconds],
      total_action_latency_seconds: execution && execution[:total_action_latency_seconds],
      slow_step: execution && execution[:slow_step],
