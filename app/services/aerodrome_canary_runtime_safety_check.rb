class AerodromeCanaryRuntimeSafetyCheck
  BANNER = "AERODROME CANARY RUNTIME SAFETY — READ ONLY"
  ...
  def check_env
    ...
    check_boolean(:env, "AERODROME_PRODUCTION_CANARY_ENABLED", true)
  ...
  def check_history
    ...
    add_check(:history, "No failed WETH during canary", recent.none? { |rebalance| weth_rebalance?(rebalance) && failed?(rebalance) }, blocker: true)
    add_check(:history, "No successful USDC during canary", recent.none? { |rebalance| rebalance.asset == "USDC" && success?(rebalance) }, blocker: true)
  end

  def check_previous_run
    return if @during_finalization

    final_event = latest_canary_final_event
  ...
  def latest_canary_final_event
    path = Dir.glob(@log_dir.join("*.jsonl")).max_by { |candidate| File.mtime(candidate) }
