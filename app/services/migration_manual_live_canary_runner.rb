class MigrationManualLiveCanaryRunner
  ...
  def from_executor_result(result)
    ...
    confirmed = result.status == "success" && receipt[:source_flat_confirmed] && receipt[:target_holds_hedge_confirmed] && receipt[:final_inside_tolerance]
    {
      ...
      pre_window_warmup_started_at: receipt[:pre_window_warmup_started_at],
      pre_window_warmup_finished_at: receipt[:pre_window_warmup_finished_at],
      pre_window_warmup_reads: receipt[:pre_window_warmup_reads],
      pre_window_warmup_error: receipt[:pre_window_warmup_error],
