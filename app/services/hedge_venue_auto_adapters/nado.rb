  ...
  class Nado < Base
    ...
    def readiness(position:)
      ...
      report = base_report(
        ...
        extra_blockers: nado_blockers(current: current, account_state: account_state, market_metadata: market_metadata),
        warnings: nado_warnings
      )
      report.merge(
    ...
    def nado_blockers(current:, account_state:, market_metadata:)
      blockers = []
      blockers << "AERODROME_NADO_AUTO_REBALANCE_ENABLED must be true" unless bool_env("AERODROME_NADO_AUTO_REBALANCE_ENABLED")
    ...
    def nado_warnings
      if bool_env("AERODROME_NADO_AUTO_REBALANCE_ENABLED")
