...
Cap tiers are separate. `production_canary_run`, `live_observation_window`, and `production_target_step_test` remain micro-capped at `0.02` ETH / `$50`. `production_live_run` V1 may use the higher supervised production tier only with explicit hard-ceiling env and runtime caps: set `AERODROME_PRODUCTION_HARD_MAX_SHORT_ETH=1.5`, a matching `AERODROME_PRODUCTION_HARD_MAX_SHORT_NOTIONAL_USD`, and `AERODROME_PRODUCTION_HARD_EMERGENCY_CLOSE_MAX_ETH`; then set `AERODROME_MAX_SHORT_ETH`, `AERODROME_MAX_SHORT_NOTIONAL_USD`, and `AERODROME_LIVE_EMERGENCY_CLOSE_MAX_ETH` at or below those hard ceilings. Leverage remains exactly `1`, min order notional remains at least `$10`, and emergency close max ETH must be at least the configured max short ETH. Increasing caps further requires separate review.

The Aerodrome position dashboard shows configured runtime caps, hard ceilings, current read-only Hyperliquid ETH position, current LP WETH amount, target hedge, drift, cap status, blockers, and a stop/close plan. The panel is read-only; it does not submit orders or run emergency close.

The first real new-position 1x supervised run passed and is documented in `docs/AERODROME_NEW_POSITION_FIRST_LIVE_RUN_REPORT.md`. Aerodrome Slipstream token id `70184676` opened WETH hedge `#200` from `0.0` to `0.3912` ETH under `0.55 ETH` / `$1300` production caps, runtime safety stayed `PASS`, approved-open monitoring worked, and watchdog was `WARN` with no blockers rather than `BLOCKED`. The operator manually ran the gated emergency close afterward, closing mainnet ETH to nil. Keep the web/dashboard running during live runs so PnL and status remain visible through the SSH tunnel.

The first 6-hour supervised run on the new real 1x position passed and is documented in `docs/AERODROME_NEW_POSITION_6H_LIVE_RUN_REPORT.md`. It ran 21600 seconds with 72 iterations, kept web/dashboard online, opened WETH hedge `#201` from `0.0` to `0.3973` ETH, kept runtime safety `PASS`, and finished duration-complete with `position_left_open=true`, `final_position_confirmed=true`, `manual_action_required=false`, and no errors. Approved-open monitoring should treat the open in-cap ETH hedge as monitored state, not emergency state. Emergency close remains manual and gated. This is supervised production only, not unattended 24/7 approval.

During the 6-hour run, natural rebalances did not execute even when some proposed deltas exceeded `$10` because `hedge.tolerance=0.05` likely kept relative deviation inside tolerance. This anti-churn policy is protective; review rebalance tolerance separately before changing it.
