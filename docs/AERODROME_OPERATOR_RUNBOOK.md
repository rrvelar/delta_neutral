...
- A read-only AERO Rewards section. When `AERODROME_REWARDS_ENABLED=true`, it runs the read-only rewards check and shows status, staked state, claimable AERO amount, depositor address/source, gauge address, and token id. If config or RPC is unavailable, the page shows unavailable without crashing. AERO USD value remains unavailable, claiming/staking is not implemented, and rewards are not included in Total PnL.
