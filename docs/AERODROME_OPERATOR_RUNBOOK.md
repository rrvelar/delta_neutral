...
Cap tiers are separate. `production_canary_run`, `live_observation_window`, and `production_target_step_test` remain micro-capped at `0.02` ETH / `$50`. `production_live_run` V1 may use the higher supervised production tier only with explicit env caps: max short ETH `<= 0.75`, max short notional `<= $2000`, leverage exactly `1`, min order notional at least `$10`, and emergency close max ETH at least the configured max short ETH. The current first real direct Slipstream position should use `AERODROME_MAX_SHORT_ETH=0.55`, `AERODROME_MAX_SHORT_NOTIONAL_USD=1300`, and `AERODROME_LIVE_EMERGENCY_CLOSE_MAX_ETH=0.60`. Increasing caps further requires separate review.
