...
AERO rewards are read-only discovery only. The dashboard does not include rewards in Total PnL, and `bin/rails aerodrome:rewards_check` does not claim, approve, transfer, or send transactions. The rewards check uses the position wallet/depositor address plus token id and reports it separately from the discovered gauge address; `not_staked` means the token id was not found for that depositor in the discovered CL gauge. Claiming and staking are not implemented. AERO USD valuation is a separate future task, so reward USD value remains unavailable until a verified price source is added. Live remains disabled.
