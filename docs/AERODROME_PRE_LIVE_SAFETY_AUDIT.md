...
AERO rewards are read-only discovery only. The dashboard does not include rewards in Total PnL, and `bin/rails aerodrome:rewards_check` does not claim, approve, transfer, or send transactions. When `AERODROME_REWARDS_ENABLED=true`, the Aerodrome position page also runs the same read-only check and displays status, staked state, claimable AERO amount, depositor address/source, gauge address, and token id; config/RPC failures render as unavailable without breaking the page. For staked Slipstream NFTs, the stored/owner wallet can be the gauge, so set `AERODROME_REWARDS_DEPOSITOR_ADDRESS` to the real staking wallet to read earned rewards. When unset, the check falls back to `position.wallet.address`. It reports `position_wallet_address`, compatibility `wallet_address`, `depositor_address`, `depositor_source`, and `gauge_address`; `not_staked` means the token id was not found for that selected depositor in the discovered CL gauge. Claiming and staking are not implemented. AERO USD valuation is a separate future task, so reward USD value remains unavailable until a verified price source is added. Live remains disabled.
