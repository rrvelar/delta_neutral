...
| Aerodrome testnet close-path rehearsal | BLOCKED PENDING RETEST | The close path exposed a false-success bug: `close_short` could return nil while `HedgeSyncJob` recorded `new_short_size=0` success. Live remains blocked until the close-path fix is tested on testnet. |
...
Testnet rehearsal found that the open path can submit a tiny WETH/ETH short, but the close path previously logged `No open position to close for ETH` and still recorded a successful `ShortRebalance` with `new_short_size=0`. False successful close rebalances created before the close-path fix must not be trusted as evidence that a Hyperliquid short was closed. Live remains blocked until the fixed close path is retested and verified on testnet.
