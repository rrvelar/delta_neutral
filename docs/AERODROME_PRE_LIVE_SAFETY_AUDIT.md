...
| Aerodrome testnet close-path rehearsal | BLOCKED PENDING RETEST | The close path exposed a false-success bug: SDK `market_close` returned nil/`No open position found for ETH` for an API-wallet/master-account short while the short remained open. `HedgeSyncJob` now passes the known `current_short` size, and `close_short` submits an explicit opposite market order. Live remains blocked until this explicit close path is tested on testnet. |
