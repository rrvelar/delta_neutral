...
AERO rewards are read-only discovery only. The dashboard does not include rewards in Total PnL, and `bin/rails aerodrome:rewards_check` does not claim, approve, transfer, or send transactions. Claiming is not implemented. AERO USD valuation is a separate future task, so reward USD value remains unavailable until a verified price source is added. Live remains disabled.
