...
| Live observation window task | PASS | `aerodrome:live_observation_window` is a strictly gated one-off tool, not background automation. It is blocked by default, capped at 30 minutes, requires close-on-finish, requires emergency close gates, and records JSONL observation events. |
...
`bin/rails aerodrome:live_observation_window` exists for that possible next stage, but it remains blocked by default and live remains disabled by default. The task requires manual live gates, observation confirmation, max 30 minute duration, minimum 60 second interval, max leverage 1, max short ETH no more than `0.02`, max notional no more than `50`, mandatory close-on-finish, and live emergency close gates. It is not daemon automation, not UI execution, and not approval to scale.
