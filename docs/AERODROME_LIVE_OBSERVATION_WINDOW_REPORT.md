...
The live observation guard now allows a separately approved supervised 3-hour window, but it remains a one-off manual tool, not continuous unattended operation. Close-on-finish, final readback retries, and emergency close gates are still mandatory, and risk caps remain unchanged at max `0.02` ETH, max `$50` notional, and `1x` leverage unless a future review explicitly changes them. For 3-hour runs, prefer interval >= 300 seconds, actively watch the run, and verify final mainnet ETH is nil.
...
A later supervised 3-hour live observation window has also passed and is recorded in `docs/AERODROME_3H_LIVE_OBSERVATION_REPORT.md`. `ShortRebalance #188` opened a tiny mainnet ETH short (`0.011`), iterations 2 through 36 created no additional rebalances, the gated live emergency close closed the short, final mainnet ETH position was nil, `final_position_confirmed=true`, and `manual_action_required=false`. This still is not approval for continuous unattended live operation. The next stage should be production supervised mode planning: watchdogs, alerts, healthchecks, logs, VPS/uptime, and clear stop/close rules.
