...
The Aerodrome position dashboard now shows the runtime hedge cap, production hard ceiling, read-only current Hyperliquid ETH position, current LP WETH amount, target hedge, drift, cap status, and blockers. It also has one-shot preview/live controls for open, rebalance, and close. Preview is dry-run. Live open/rebalance is blocked unless the dashboard live gates and typed confirmation are present, and it reuses the existing `HedgeSyncJob` path. Live close is routed through the existing gated emergency close service. These controls do not schedule `production_live_run`, do not add tmux queues, and do not create unattended operation.
