...
Production live V1 has a higher supervised cap tier than the micro tools. Canary, observation, and target-step runs stay capped at `0.02` ETH / `$50`, but `production_live_run` may use explicitly configured runtime caps up to the configured production hard ceilings at `1x`. For the larger supervised LP/hedge size, set `AERODROME_PRODUCTION_HARD_MAX_SHORT_ETH=1.5`, a matching hard notional ceiling, and a hard emergency close ceiling; then set runtime `AERODROME_MAX_SHORT_ETH`, `AERODROME_MAX_SHORT_NOTIONAL_USD`, and `AERODROME_LIVE_EMERGENCY_CLOSE_MAX_ETH` separately. The VPS deploy still does not approve unattended 24/7 operation, and any cap increase beyond the configured hard ceiling requires separate review.

The Aerodrome position dashboard is the normal status surface for supervised operation. It shows the configured runtime caps, hard ceilings, current Hyperliquid ETH readback, LP WETH amount, target hedge, drift, cap blockers, and stop/close guidance. It is read-only and does not submit live orders.

The first real new-position 1x supervised VPS run passed. Token id `70184676` opened WETH hedge `#200` at `0.3912` ETH, approved-open monitoring reported `approved`, `production_live_status` was `PASS`, and `watchdog_alerts` was `WARN` with no blockers. The operator manually ran the gated emergency close afterward and final mainnet ETH readback was nil. For production-supervised live runs, keep the web container running and launch the runner in a one-off container so the dashboard/PnL remains available through the SSH tunnel.

The first 6-hour supervised VPS run on token id `70184676` also passed and is documented in `docs/AERODROME_NEW_POSITION_6H_LIVE_RUN_REPORT.md`. It ran 21600 seconds with 72 iterations while web/dashboard remained online, opened WETH hedge `#201` at `0.3973` ETH, kept runtime safety `PASS`, and finished successfully with the ETH hedge left open by design. Approved-open monitoring should validate the in-cap hedge while open, watchdog should have no blockers, and emergency close remains a separate manually gated procedure. This does not approve unattended VPS live service.

The 6-hour run also showed that `hedge.tolerance=0.05` can suppress additional natural rebalances even when proposed delta notional is above `$10`. Do not change tolerance as part of deployment operations; perform a separate rebalance policy review first.
