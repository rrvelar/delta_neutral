...
class AerodromeProductionCanaryRunnerTest < ActiveSupport::TestCase
  setup do
    @env = {
      ...
      "AERODROME_PRODUCTION_CANARY_CONFIRM" => AerodromeProductionCanaryRunner::CONFIRMATION,
      "AERODROME_PRODUCTION_CANARY_DURATION_SECONDS" => "180",
      "AERODROME_PRODUCTION_CANARY_INTERVAL_SECONDS" => "180",
    ...
    with_position_and_hedge do
      {
        ...
        "AERODROME_PRODUCTION_CANARY_CONFIRM" => "wrong",
        "AERODROME_PRODUCTION_CANARY_CLOSE_ON_FINISH" => "false"
      }.each do |key, value|
  ...
  test "blocks hard risk limits" do
    with_position_and_hedge do
      {
        "AERODROME_PRODUCTION_CANARY_DURATION_SECONDS" => "21601",
        "AERODROME_PRODUCTION_CANARY_INTERVAL_SECONDS" => "179",
  ...
  test "blocks if mainnet ETH position exists before start" do
    with_position_and_hedge do
      with_env(@env) do
        report = build_service(hyperliquid_service: HyperliquidReadMock.new(positions: [ eth_position("-0.011") ])).report

        assert_equal "blocked", report.fetch(:status)
        assert_includes report.fetch(:errors), "mainnet ETH position must be nil before canary start"
      end
    end
  end
    ...
    with_position_and_hedge do
      ...
      File.open(lock_path, File::RDWR | File::CREAT, 0o644) do |file|
        ...
        with_env(@env) do
          ...
          assert_equal "blocked", report.fetch(:status)
          assert_includes report.fetch(:errors), "another production canary runner is active"
        end
      end
    end
  end

  test "successful mocked canary run opens maintains closes and final nil" do
    with_position_and_hedge do |hedge|
      ...
      with_env(@env) do
        ...
        assert_equal 1, report.fetch(:iterations)
  ...
  test "stop on ETH exceeds max" do
    with_position_and_hedge do |hedge|
      ...
      with_env(@env) do
        ...
        assert_equal "failed", report.fetch(:status)
        assert_equal "actual ETH short exceeds max", report.fetch(:stop_reason)
      end
    end
  end
  ...
  test "signal handling triggers final close" do
    with_position_and_hedge do
  ...
  test "final readback unknown returns close unknown and manual action" do
    with_position_and_hedge do |hedge|
      hyperliquid = HyperliquidReadMock.new(positions: [ nil, eth_position("-0.011"), eth_position("-0.011"), OpenSSL::SSL::SSLError.new("SSL_read") ])
      hedge_sync = ->(_) { create_success_rebalance(hedge) }
  ...
  def tmp_path(name)
    Rails.root.join("tmp", "aerodrome_canary_test", SecureRandom.hex(4), name)
  end
