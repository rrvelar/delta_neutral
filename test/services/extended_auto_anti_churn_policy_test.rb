...
class ExtendedAutoAntiChurnPolicyTest < ActiveSupport::TestCase
  ...
  test "cooldown suppresses normal drift" do
    ...
    result = policy(env: permissive_env).evaluate(position: position, hedge: position.hedge, action: "increase_short", drift: "0.015".to_d, tolerance: "0.01".to_d, order_size: "0.015".to_d, mark_price: "2000".to_d)

    assert_match "cooldown", result.fetch(:action_suppressed_reason)
    assert_operator result.fetch(:cooldown_remaining_seconds), :>, 0
    assert_equal false, result.fetch(:strong_drift_bypass_used)
  end

  test "strong drift bypasses cooldown" do
    position.hedge.short_rebalances.create!(venue: "extended", asset: "ETH", old_short_size: "0.8", new_short_size: "0.85", status: ShortRebalance::STATUS_SUCCESS, rebalanced_at: 1.minute.ago)

    result = policy(env: permissive_env).evaluate(position: position, hedge: position.hedge, action: "increase_short", drift: "0.02".to_d, tolerance: "0.01".to_d, order_size: "0.02".to_d, mark_price: "2000".to_d)

    assert_nil result.fetch(:action_suppressed_reason)
    assert_operator result.fetch(:cooldown_remaining_seconds), :>, 0
    assert_equal true, result.fetch(:strong_drift_bypass_used)
    assert_equal "2.0", result.fetch(:drift_to_tolerance_ratio)
    assert_equal "0.02", result.fetch(:strong_drift_threshold)
  end

  test "consecutive requirement suppresses normal drift" do
  ...
  test "strong drift bypasses consecutive requirement" do
  ...
  test "below min order still blocks invalid tiny order even when drift is strong" do
    result = policy(env: permissive_env.merge("EXTENDED_AUTO_MIN_REBALANCE_SIZE_ETH" => "0.03")).evaluate(
      position: position,
      hedge: position.hedge,
      action: "increase_short",
      drift: "0.02".to_d,
